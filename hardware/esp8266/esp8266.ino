...
#define DIGITAL_LEFT_PIN D5
#define DIGITAL_RIGHT_PIN D6
#define DIGITAL_TOP_PIN D7
#define DIGITAL_BOTTOM_PIN D8
...
void stop() {
  ...
  digitalWrite(DIGITAL_TOP_PIN, LOW);
  digitalWrite(DIGITAL_BOTTOM_PIN, LOW);
}
...
void handleController(AsyncWebServerRequest *request) {
  if(request->hasParam("btn") && request->hasParam("state")) {
    ...
    if(btn == "left")
      digitalWrite(DIGITAL_LEFT_PIN, state);
    if(btn == "right")
      digitalWrite(DIGITAL_RIGHT_PIN, state);
    if(btn == "top")
      digitalWrite(DIGITAL_TOP_PIN, state);
    if(btn == "bottom")
      digitalWrite(DIGITAL_BOTTOM_PIN, state);
...
void setup() {
  ...
  pinMode(DIGITAL_TOP_PIN, OUTPUT);
  pinMode(DIGITAL_BOTTOM_PIN, OUTPUT);
}
